...
def get_usage(username: str | None = None, password: str | None = None):
...
def term_main():
    ...
    username = input("Enter username(pre-saved): ")
    if username == "":
        username = "0777637002"

    # with open(scriptpath.parent / "assets/wifi_hash.txt", "r") as f:
    #     target_hash = f.read()
    #     print(target_hash)

    target_hash = "5a85290a7d94d94077a0c2842455f1859d1a1882d4f1889cd6e0878019930eec"

    valid_pass = False
    tries = 0
    while tries < 5:
        password = getpass("Enter password: ")
        entry_pass = hashlib.sha256(password.encode()).hexdigest()
        if entry_pass == target_hash:
            valid_pass = True
            break
        else:
            os.system("cls")
            print(click.style("INVALID PASSWORD\n", fg=(255, 0, 0)))
            tries += 1

    if valid_pass:
        wifi_data = get_usage(username=username, password=password)
        date_data = get_date()
        app = App(
            size=(850, 450),
            wifi_data=wifi_data,
            ...
            # ),
            iconphoto=scriptpath.parent / "assets/g6.png",
        )
        ...
        print(click.style("INVALID LOGIN INFO!\nTERMINATING PROCESS", fg=(255, 1, 1)))
...
class MeterFrame(ttk.Frame):
    def __init__(self, master, label_text: str, wifi_data, date_data) -> None:
        ...
        self.average_used_lbl = ttk.Label(
            self, text=f"Average used: 10000", font=SIDE_METER_FONT
        )
        self.average_used_lbl.place(relx=0.5, rely=0.65, anchor="center")

        self.target_average_lbl = ttk.Label(
            self, text=f"Target average: 10000", font=SIDE_METER_FONT
        )
        self.target_average_lbl.place(relx=0.5, rely=0.72, anchor="center")

        self.available_average_lbl = ttk.Label(
            self, text=f"Available average: 10000", font=SIDE_METER_FONT
        )
